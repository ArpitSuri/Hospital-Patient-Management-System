...
const ServicesPreview = () => {
  ...
  return (
    <section
      ref={ref}
      className="relative py-24 overflow-hidden bg-gradient-to-br from-blue-50 via-purple-50 to-white"
    >
      {/* Animated background blobs */}
      <div className="absolute -top-20 -left-20 w-96 h-96 bg-purple-100 rounded-full mix-blend-multiply filter blur-2xl opacity-30 animate-pulse"></div>
      <div className="absolute -bottom-20 -right-20 w-96 h-96 bg-blue-100 rounded-full mix-blend-multiply filter blur-2xl opacity-30 animate-pulse"></div>
      ...
      <div className="relative container mx-auto px-4">
        ...
        >
          ...
          <motion.h2
            className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-8 leading-tight tracking-tight"
            variants={item}
          >
            We provide <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-purple-600">exceptional care</span> for your smile.
          </motion.h2>
        ...
        <motion.div
          ...
          animate={inView ? "visible" : "hidden"}
        >
          {approachItems.map((itemData, index) => (
            <motion.div
              key={index}
              className="bg-white rounded-3xl p-8 text-center hover:shadow-2xl transition-all duration-300 border border-transparent hover:border-blue-100 hover:scale-[1.02]"
              variants={item}
              whileHover={{ y: -10 }}
            >
              <motion.div
                className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-blue-200 to-purple-200 flex items-center justify-center shadow-md"
                whileHover={{ rotate: 5, scale: 1.1 }}
              >
                <span className="text-3xl">{itemData.icon}</span>
              </motion.div>
              <h3 className="text-xl font-bold text-gray-900 mb-4">{itemData.title}</h3>
        ...
        >
          <motion.button
            className="relative px-8 py-4 rounded-full bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
